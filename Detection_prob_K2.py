...
def properties_GAP(X, constants_only=False):
    # solar parameters
    # teff_solar = 5777.0 # Kelvin
    teffred_solar = 8907.0 #in Kelvin
    # numax_solar = 3104 # in micro Hz
    ...
    X['Tred'] = teffred_solar*(X['Lumo']**-0.093) # from (3) eqn 8. red-edge temp
...
def keplerNoise(Kp):
    ...
    noise = np.sqrt(2) * 1e6/c * np.sqrt(c + 9.5 * 1e5*(14./Kp)**5) # from (4) eqn 17. in ppm
    return noise
